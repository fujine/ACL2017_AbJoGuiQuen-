...
=======
    ...
    /**
     * Constructeur Ã  partir d'une position et d'un plateau avec dÃ©finition de la vie du hÃ©ro
     * @param coord CoordonnÃ©e du hÃ©ro sur le plateau
     * @param plateau Plateau au qu'elle appartient le hÃ©ro
     */
    public Hero(Point coord, Plateau plateau) {
        super(coord, plateau);
        dir = Direction.BAS;
    ...
    /**
     * VÃ©rifie les coordonnÃ©es avant de deplacer le hero au coordonnÃ©es donnÃ©es
     * @param coord futur position du hÃ©ro
     */
    public void deplacer(Point coord,Direction direction) {
        Point coordBG = new Point(coord.x,coord.y+Jeu.TAILLE-1);
        Point coordBD = new Point(coord.x + Jeu.TAILLE-1,coord.y+Jeu.TAILLE-1);
        ...
        Jeu mod = Jeu.getInstance();
        if (coordBG.x >= 0 && coordBG.y>= 0 && coordBD.x < plateau.getLargeur() && coordBD.y < plateau.getHauteur() ){
            //Test de colision et de zone libre pour le deplacement du hÃ©ro
            if(plateau.estLibre(coordBG) && plateau.estLibre(coordBD) && plateau.estLibre(coordHD) && plateau.estLibre(coordHG) && mod.collisionEntites(this,colli) == null) {
                this.coord = coord;
                getPlateau().appliquerEffetCase(coordBD);
                getPlateau().appliquerEffetCase(coordBG);
    ...
    /**
     * Diminue la vie du hÃ©ro en fonction du nombre de dÃ©gats subit
     * @param nbDegats
     */
    public void subirDegat(int nbDegats) {
        this.vie = vie - nbDegats;
    ...
    /**
     * Verifie que le hÃ©ro soit encore en vie
     * @return true si le hÃ©ro a plus de 0 de vie, false sinon
     */
    public boolean estVivant() {
        return this.vie <= 0;
    }
